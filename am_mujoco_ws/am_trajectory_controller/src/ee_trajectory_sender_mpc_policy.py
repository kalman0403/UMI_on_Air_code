...
import rospy
from std_msgs.msg import Bool, Float64MultiArray, Float32
...
from planner.ee_ik_DH_4dof import init_ik_DH, WAITING, FREEFLIGHT, TAKEOFF, LAND, PAUSE
...
class TrajectorySender:
    def __init__(self):
        ...
        self.odom = None
        self.mocap = None
        # Live policy-driven EE horizon buffer (N x 7), XYZW quaternion order
        self.horizon_buf = None
        self.gripper_target = 0.0
        # Latest IK target and timestamp
        self._ik_target = None  # shape (7,) [px,py,pz,qx,qy,qz,qw]
        ...
        # Buffer used for preplanned sequences (takeoff/land/pause)
        self.traj = []

        self.t_bar = tqdm(total=0)
        self.takeoff = False
        self.use_mpc_cnt = -1
        ...
        # Match policy sender publishers for EE target/state visibility
        ...
        # Live EE horizon (N x 7: pos[3], quat_xyzw[4])
        ...
        self.gripper_sub = rospy.Subscriber(
            "/policy/ee_gripper_target", Float32, self.gripper_callback, queue_size=5
        )
        # Subscribe to single-step IK EE target to optionally override MPC
        self.ee_target_sub = rospy.Subscriber(
            "/policy/ee_target", PoseCtrlTarget, self.ee_target_callback, queue_size=5
        )
        # Roll DOF for wrist rotation
        self.rolldof_target = 0.0
        self.rolldof_target_sub = rospy.Subscriber(
            "/policy/ee_rolldof_target", Float32, self.rolldof_target_callback, queue_size=5
        )
    ...
    def ee_horizon_callback(self, msg: Float64MultiArray):
        ...
        if D != 7:
            ...
            return
        if N != N_expected:
            rospy.logwarn_throttle(1.0, f"EE horizon length {N} != MPC.N {N_expected}. Ignoring.")
            return
        horizon = data.reshape((N, 7))
        # Optional: clip positions to geofence for safety
        pos = horizon[:, :3]
        pos = np.clip(pos, self.geofence_min, self.geofence_max)
        horizon[:, :3] = pos
    ...
    def gripper_callback(self, msg: Float32):
        self.gripper_target = float(msg.data)

        
        
    def rolldof_target_callback(self, msg: Float32):
        self.rolldof_target = float(msg.data)

    def ee_target_callback(self, msg: PoseCtrlTarget):
        # Store the latest IK target and timestamp
        ...
        self._ik_target = np.concatenate([pos, quat])
    ...
    def odom_callback(self, msg):
        self.odom = msg
        # Publish EE state similar to policy sender
        base_pos = np.array([
        ...
        base_euler = R.from_quat([base_quat_wxyz[1], base_quat_wxyz[2], base_quat_wxyz[3], base_quat_wxyz[0]]).as_euler('zyx', degrees=False)[::-1]
        u = np.concatenate([base_pos, base_euler, self.arm_joints])
    ...
    def start(self):
        ...
        while not rospy.is_shutdown():
            ...
            # If a recent IK target arrived more recently than MPC horizon, use IK for this step
            use_ik_now = False
            if self._ik_target is not None:
                if getattr(self, "_horizon_time", None) is None:
                    use_ik_now = True
                else:
                    use_ik_now = (self._ik_target_time > self._horizon_time)

            if use_ik_now:
                # Current base pose (position xyz and quaternion xyzw)
                current_base_pos = np.array([
                    self.odom.pose.pose.position.x,
                    self.odom.pose.pose.position.y,
                    self.odom.pose.pose.position.z,
                ])
                current_base_quat_xyzw = np.array([
                    self.odom.pose.pose.orientation.x,
                    self.odom.pose.pose.orientation.y,
                    self.odom.pose.pose.orientation.z,
                    self.odom.pose.pose.orientation.w,
                ...
                target_pos = self._ik_target[:3]
                target_quat_xyzw = self._ik_target[3:7]

                ret = ee_pos_to_base_pos_and_arm_joints_4dof(
                    self.ik_planner,
                    target_pos,
                    target_quat_xyzw,
                    current_base_pos,
                ...
                if ret is None:
                    raise ValueError("IK solver failed for target EE pose")

                km_target = ret
                target_base_pos = km_target[:3]
                target_arm_joints_deg = km_target[7:10] * RAD_TO_DEG

                # Publish base target (keep orientation identity here to match policy sender behavior)
                msg.header.stamp = rospy.Time.now()
                msg.pose.position = Point(*(target_base_pos))
                msg.pose.orientation = Quaternion(0.0, 0.0, 0.0, 1.0)
                if self.send_vel:
                    msg.twist.linear = Point(0.0, 0.0, 0.0)
                if self.send_thrust:
                    msg.thrust = Vector3(0.0, 0.0, 0.0)
                self.traj_pub.publish(msg)

                if self.send_arm:
                    # Include roll DOF (deg) as 4th joint like policy sender
                    joint = np.array([
                        target_arm_joints_deg[0],
                        target_arm_joints_deg[1],
                        target_arm_joints_deg[2],
                        self.rolldof_target * RAD_TO_DEG,
                    ])
                    cmd = np.concatenate([joint, np.array([self.gripper_target])])
                    arm_msg.position = cmd.tolist()
                    self.arm_pub.publish(arm_msg)

                # Publish ee tracking targets to mirror policy sender
                ee_tracking_target_msg = PoseCtrlTarget()
                ee_tracking_target_msg.header.stamp = rospy.Time.now()
                ee_tracking_target_msg.pose.position = Point(*target_pos)
                ee_tracking_target_msg.pose.orientation = Quaternion(*target_quat_xyzw)
                self.ee_traj_pub.publish(ee_tracking_target_msg)

                # Publish the realized EE target estimated from IK result
                real_msg = PoseCtrlTarget()
                real_msg.header.stamp = rospy.Time.now()
                # Compute real EE pose from km_target
                ee_state_real = self.ik_planner.forward_kinematics(km_target, base_rpy_use=False)
                real_msg.pose.position = Point(*ee_state_real[:3])
                real_msg.pose.orientation = Quaternion(*ee_state_real[3:])
                self.real_ee_tracking_target_pub.publish(real_msg)
            ...
            if self.use_mpc_cnt == 0:
                # Use live horizon buffer if available, else hold current pose
                if self.horizon_buf is not None and self.horizon_buf.shape == (self.planner.mpc.N, 7):
                    p_ref = self.horizon_buf[:, :3]
                    quat_ref_xyzw = self.horizon_buf[:, 3:7]
                else:
                    p_ref = np.tile(current_ee_pos.reshape(1, 3), (self.planner.mpc.N, 1))
                    quat_ref_xyzw = np.tile(np.array([0.0, 0.0, 0.0, 1.0]).reshape(1, 4), (self.planner.mpc.N, 1))
                ee_pos_ref = p_ref[0]
                ee_quat_ref = quat_ref_xyzw[0]
                quat_ref = xyzw_to_wxyz(quat_ref_xyzw)
                ...
                joint = np.array([arm_angle[0], arm_angle[1], arm_angle[2], self.rolldof_target * RAD_TO_DEG])
                gripper = np.array([self.gripper_target])
                ...
                joint = np.array([self.arm_default_angle[0], self.arm_default_angle[1], self.arm_default_angle[2], self.rolldof_target * RAD_TO_DEG]) 
                gripper = np.array([self.gripper_target])

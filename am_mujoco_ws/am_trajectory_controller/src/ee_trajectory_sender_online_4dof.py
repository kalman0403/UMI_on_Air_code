...
from planner.ee_ik_DH_4dof import WAITING, FREEFLIGHT, TAKEOFF, LAND, PAUSE, UAVMODE
...
class TrajectorySender:
    def __init__(self):
        ...
        self.ee_target_sub = rospy.Subscriber("/gamepad/ee_target", PoseCtrlTarget, self.ee_target_callback, queue_size=10)
        ...
        self.ee_gripper_target_sub = rospy.Subscriber("/gamepad/gripper_target", Float32, self.ee_gripper_target_callback, queue_size=5)
        ...
        self.rolldof_target_sub = rospy.Subscriber("/gamepad/ee_rolldof_target", Float32, self.rolldof_target_callback, queue_size=5)

        # uav mode
        self.uavmode_sub = rospy.Subscriber("/gamepad/uav_mode", Bool, self.uavmode_callback, queue_size=5)
        self.uav_mode = False
    ...
    # Xiaofeng@202501172205: Haven't tested this
    ...
    def ee_target_callback(self, msg):
        ...
        # Xiaofeng@202501212308 TODO: check this
        # print("self.traj: ", self.traj)
        # self.traj = [self.traj[0], ee_target_state]
        self.traj = [ee_target_state]
    ...
    def start(self):
        ...
        while not rospy.is_shutdown():
            ...
            # ee_ori: x,y,z,w
            # true_ee_ori = new_waypoint[3:]
            # fake_ee_ori = np.array([0.0, 0.0, 0.0, 1.0])
            ...
            if self.uav_mode and self.state_status == FREEFLIGHT:
                self.state_status = UAVMODE
            if not self.uav_mode and self.state_status == UAVMODE:
                self.state_status = FREEFLIGHT

            target_base_pos, target_base_quat, target_arm_joints, real_target_ee_state = self.get_reference_state_fulldof(self.ik_planner,
                                                                                                    ee_pos,
                                                                                                    ee_ori,
                                                                                                    cur_base_pos,
                                                                                                    cur_base_quat,
            ...
            # fake_third_pitch = self.get_third_pitch_fake(true_ee_ori)
            ...
            if self.send_arm:
                if self.state_status != UAVMODE:
                    ...
                    # print("takeoff joint angles", self.ik_planner.takeoff_arm_joints)

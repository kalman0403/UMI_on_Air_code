...
def ee_pos_to_base_pos_and_arm_joints_2dof(ik, target_ee_pos, target_ee_ori, cur_base_pos, cur_base_ori, cur_joints, drone_state):
    '''
    input: 
    ...
    output:
    km_target: [base_pos, base_ori_x, base_ori_y, base_ori_z, base_ori_w, joint1, joint2]
    '''    
    ...
    try:
        temp_km_target = ik.optimize(current_qpos, target_ee_pos, target_ee_quat, drone_state)
        km_target = np.array([temp_km_target[0], temp_km_target[1], temp_km_target[2], 
                              temp_km_target[4], temp_km_target[5], temp_km_target[6], temp_km_target[3],
                                temp_km_target[7], temp_km_target[8]])
    except:
        # km_target = np.array([cur_base_pos[0], cur_base_pos[1], cur_base_pos[2],
        #                         cur_base_ori[0], cur_base_ori[1], cur_base_ori[2], cur_base_ori[3],
        #                         cur_joints[0], cur_joints[1]])
        km_target = None
...
def ee_pos_to_base_pos_and_arm_joints_4dof(ik, target_ee_pos, target_ee_ori, cur_base_pos, cur_base_ori, cur_joints, drone_state):
    ...
    output:
    km_target: [base_pos, base_ori_x, base_ori_y, base_ori_z, base_ori_w, joint1, joint2, joint3, joint4]
    '''    
    # print("cur_joints", cur_joints)
    # print("ee_pos_target", ee_pos_target)
    current_qpos = np.zeros(11)
    ...
    try:
        ...
        km_target = np.array([temp_km_target[0], temp_km_target[1], temp_km_target[2], 
                              temp_km_target[4], temp_km_target[5], temp_km_target[6], temp_km_target[3],
                                temp_km_target[7], temp_km_target[8], temp_km_target[9], temp_km_target[10]])
    except:
        # km_target = np.array([cur_base_pos[0], cur_base_pos[1], cur_base_pos[2],
        #                         cur_base_ori[0], cur_base_ori[1], cur_base_ori[2], cur_base_ori[3],
        #                         cur_joints[0], cur_joints[1]])
...
#     """
#     Convert Euler angles to a rotation matrix using CasADi symbolic operations.

#     Parameters:
#     euler (casadi.SX or casadi.MX): A 3-element vector of Euler angles [roll, pitch, yaw].

#     Returns:
#     R (casadi.SX or casadi.MX): A 3x3 rotation matrix.

#     Note by xiaofeng 11/07/2024:
#     the calculation is in roll-pitch-yaw order, but need to be double-checked
#     """

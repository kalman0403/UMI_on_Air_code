...
def init_ik_Mj(xml_path, cur_base_pos, cur_base_quat, cur_joints, visualize):
    ik = IKPlanner(xml_path, visualize=visualize)
...
class IKPlanner:
    ...
    def __init__(self, xml_path, ps=0.1, pz_l=0.0, pz_h=0.9,
        ...
        self.drone_states = [TAKEOFF, PAUSE, LAND, FREEFLIGHT]
        if visualize:
    ...
    def optimize(self, current_qpos: np.ndarray, ee_pos_target: np.ndarray, ee_quat_target: np.ndarray, drone_state: int):
        # Set the drone state

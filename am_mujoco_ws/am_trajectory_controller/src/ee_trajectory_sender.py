...
from planner.ee_ik_DH import FREEFLIGHT, TAKEOFF, LAND, PAUSE
...
class TrajectorySender:
    ...
    def takeoff_planner(self):
        ...
        # Interpolate the position
        start_ee_pos = start_base_pos + self.inital_ee_offset
        target_ee_pos = self.ee_traj[0, :3]
        ...
        print(self.ee_traj[0])
        target_ee_quat = self.ee_traj[0, 3:]
    ...
    def start(self):
        ...
        while not rospy.is_shutdown():
            ...
            tar_ee_pos = new_waypoint[:3]
            tar_ee_quat = new_waypoint[3:]
            ...
            # Publish arm command
            if self.send_arm:
                cmd = np.concatenate([target_arm_joints, roll_joint, gripper])

...
class TrajectorySender:
    def __init__(self, trajectory):
        ...
        # Check if the trajectory is a 18D vector
        for i in range(len(self.traj)):
            if self.traj[i].shape != (18,):
                print(self.traj[i].shape)
                raise ValueError(
                    f"{i}-th trajectory is not a 18D vector, expected 3D position, 4D quaterion, 3D velocity, 3D thrust, 3D joint angles, 1D gripper."
                )
            pos, quat, vel, thrust, joint, gripper = np.split(
                self.traj[i], [3, 7, 10, 13, 17]
            )

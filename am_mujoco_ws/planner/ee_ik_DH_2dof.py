...
class DHIKPlanner:
    ...
    def __init__(self, base_x_ub = 1.1):
        ...
        self.DH_params = np.array([[-np.pi/2, 0, 0, 0],
                                [-np.pi/2, 0.364, 0.0, -np.pi],
                                [-np.pi/2, 0.0, 0.084, 0],
                                [-np.pi/2, 0.38, 0.0, -np.pi]])
        
        self.arm_base_pos = np.array([0.1, 0.0, 0.1])
        self.ee_base_pos = np.array([0.14, 0.0, 0.0])
    ...
    def opti_init(self, base_x_ub):
        ...
        predict_ee_quat = self.predict_ee_state[3:]
        
        pose_error = ca.sumsqr(predict_ee_pos - self.obj_ee_pos_tar)*10.0
        orientation_error = (1 - ca.dot(predict_ee_quat, self.obj_ee_quat_tar)**2)*10.0
    ...
    def optimize(self, current_drone_state, target_ee_pos, target_ee_quat, drone_state_status):
        ...
        output:
        km_target: [pos_x, pos_y, pos_z, w, x, y, z, theta1, theta2]
        '''
        ...
        # print(f"Optimization time: {spend_time}")
        return km_target

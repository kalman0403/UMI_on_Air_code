...
if __name__ == "__main__":
    ...
    # Set dataset path to the peg-in-hole dataset
    dataset_path = "/home/harsh/flyingumi/data_keyboard/bc/peg_in_hole_dataset.zarr.zip"
    
    # Load configuration from umi.yaml
    ...
    # Create simplified configuration from the loaded config
    shape_meta = config['shape_meta']
    pose_repr = config['pose_repr']
    
    # Process the shape_meta to ensure proper types for numeric values
    for key in shape_meta['obs']:
        if isinstance(shape_meta['obs'][key]['horizon'], str):
            shape_meta['obs'][key]['horizon'] = 2  # Default to 2
        if isinstance(shape_meta['obs'][key]['latency_steps'], str):
            shape_meta['obs'][key]['latency_steps'] = 6.25  # Default value
        if isinstance(shape_meta['obs'][key]['down_sample_steps'], str):
            shape_meta['obs'][key]['down_sample_steps'] = 4  # Default to 4
    
    # Process action config
    if isinstance(shape_meta['action']['horizon'], str):
        shape_meta['action']['horizon'] = 8  # Default to 8
    if isinstance(shape_meta['action']['latency_steps'], str):
        shape_meta['action']['latency_steps'] = 0  # Default to 0
    if isinstance(shape_meta['action']['down_sample_steps'], str):
        shape_meta['action']['down_sample_steps'] = 4  # Default to 4
    ...
    try:
        ...
        # Sample a few items from the dataset to examine
        print("\nSampling items from dataset...")
        ...
        # Get normalizer and examine raw data statistics
        print("\nGenerating normalizer and analyzing data statistics...")
        normalizer = dataset.get_normalizer()
        
        # Collect a batch of data to analyze raw statistics
        dataloader = torch.utils.data.DataLoader(
            dataset=dataset,
            batch_size=100,
            shuffle=True,
        ...
        # Print detailed statistics for position and action data
        print("\n=== RAW DATA STATISTICS ===")
        ...
        # Sample and show normalized data
        ...
        # Print normalized statistics for the same batch
        for key in ['robot0_eef_pos', 'robot0_gripper_width']:
            if key in batch['obs']:
                # Normalize batch data
                normalized_batch = normalizer.normalize({key: batch['obs'][key]})
                flat_normalized = normalized_batch[key].reshape(-1, normalized_batch[key].shape[-1])
                
                print(f"\nNormalized {key} statistics:")
                print(f"  Mean: {flat_normalized.mean(dim=0)}")
                print(f"  Std: {flat_normalized.std(dim=0)}")
                print(f"  Min: {flat_normalized.min(dim=0).values}")
                print(f"  Max: {flat_normalized.max(dim=0).values}")
        
        # Print normalized action statistics
        normalized_action = normalizer.normalize({'action': action_data})['action']
        flat_norm_action = normalized_action.reshape(-1, normalized_action.shape[-1])
        print("\nNormalized action statistics:")
        print(f"  Mean: {flat_norm_action.mean(dim=0)}")
        print(f"  Std: {flat_norm_action.std(dim=0)}")
        print(f"  Min: {flat_norm_action.min(dim=0).values}")
        print(f"  Max: {flat_norm_action.max(dim=0).values}")
        
        # Print input stats from normalizer
        print("\n=== NORMALIZER INPUT STATISTICS ===")
        for key in ['robot0_eef_pos', 'robot0_gripper_width', 'action']:
            if key in normalizer.state_dict():
                print(f"\n{key} normalizer stats:")
                # Get the stats from normalizer state_dict
                offset = normalizer.state_dict()[f'params_dict.{key}.offset']
                scale = normalizer.state_dict()[f'params_dict.{key}.scale']
                input_min = normalizer.state_dict()[f'params_dict.{key}.input_stats.min']
                input_max = normalizer.state_dict()[f'params_dict.{key}.input_stats.max']
                input_mean = normalizer.state_dict()[f'params_dict.{key}.input_stats.mean']
                input_std = normalizer.state_dict()[f'params_dict.{key}.input_stats.std']
